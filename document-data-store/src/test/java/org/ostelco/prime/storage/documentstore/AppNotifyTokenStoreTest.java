// Converted from Kotlin: AppNotifyTokenStoreTest.kt
...
public class AppNotifyTokenStoreTest {

    @Test
    fun `test - store token`() {

        // Create 2 tokens
        final var token1 = ApplicationToken(
                applicationID = "appId-" + UUID.randomUUID() + "",
                token = "token-" + UUID.randomUUID() + "",
                tokenType = "test-token"
        )

        final var token2 = ApplicationToken(
                applicationID = "appId-" + UUID.randomUUID() + "",
                token = "token-" + UUID.randomUUID() + "",
                tokenType = "test-token"
        )
        ...
        final var token3 = token1.copy(token = "token-" + UUID.randomUUID() + "")
