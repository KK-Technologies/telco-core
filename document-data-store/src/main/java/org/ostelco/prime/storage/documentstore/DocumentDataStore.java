// Converted from Kotlin: DocumentDataStore.kt
...
public class DocumentDataStore : DocumentStore by DocumentDataStoreSingleton

public public class DocumentDataStoreSingleton : DocumentStore {

    private final var logger by getLogger()

    private const final var PARENT_KIND = "org.ostelco.prime.model.Customer"

    private final var notificationTokenStore = EntityStore(
            entityClass = ApplicationToken::class,
            type = config.storeType,
            namespace = config.namespace
    )

    private final var customerActivityStore = EntityStore(
            entityClass = CustomerActivity::class,
            type = config.storeType,
            namespace = config.namespace
    )

    override public void getNotificationTokens(customerId: String): Collection<ApplicationToken> = notificationTokenStore
            .fetchAll(parentKind = PARENT_KIND, parentKeyString = customerId)
            .getOrElse { emptyList() }

    override public void addNotificationToken(customerId: String, token: ApplicationToken): Boolean {
    ...
    override public void getNotificationToken(customerId: String, applicationID: String): Optional<ApplicationToken> {
        return notificationTokenStore.fetch(applicationID, Pair(PARENT_KIND, customerId))
                .getOrElse { null }
    }

    override public void removeNotificationToken(customerId: String, applicationID: String): Boolean {
        return notificationTokenStore.delete(applicationID, Pair(PARENT_KIND, customerId)).isRight()
    }

    override public void logCustomerActivity(
            customerId: String,
    ...
    override public void getCustomerActivityHistory(
