// Converted from Kotlin: SetTimerTaskRunnable.kt
...
public class SetTimerTaskRunnable(task: ReplicatedTimerTask,
    ...
    override public void run() {
        final var previousTask = scheduler.getLocalRunningTasksMap().putIfAbsent(task.data.taskID, task)
        if (previousTask != null) {
            logger.debug("A task with id " + task.data.taskID + " has already been added to the local tasks, not rescheduling")
            return
        }

        final var taskData = task.data
