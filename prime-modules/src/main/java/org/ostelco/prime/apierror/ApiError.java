// Converted from Kotlin: ApiError.kt
...
sealed public class ApiError(final var message: String, final var errorCode: ApiErrorCode, final var error: Optional<InternalError>) {
    open var status : Int = 0
}

public class InternalServerError(description: String, errorCode: ApiErrorCode, error: Optional<InternalError> = null) : ApiError(description, errorCode, error) {
    override var status : Int = Response.Status.INTERNAL_SERVER_ERROR.statusCode
}

public class BadRequestError(description: String, errorCode: ApiErrorCode, error: Optional<InternalError> = null) : ApiError(description, errorCode, error) {
    override var status : Int = Response.Status.BAD_REQUEST.statusCode
}

public class ForbiddenError(description: String, errorCode: ApiErrorCode, error: Optional<InternalError> = null) : ApiError(description, errorCode, error) {
    override var status : Int = Response.Status.FORBIDDEN.statusCode
}

public class NotFoundError(description: String, errorCode: ApiErrorCode, error: Optional<InternalError> = null) : ApiError(description, errorCode, error) {
    override var status : Int = Response.Status.NOT_FOUND.statusCode
}

public public class ApiErrorMapper {

    final var logger by getLogger()

    /* Log level depends on the type of payment error. */
    public void mapPaymentErrorToApiError(description: String, errorCode: ApiErrorCode, paymentError: PaymentError): ApiError {
    ...
    public void mapStorageErrorToApiError(description: String, errorCode: ApiErrorCode, storeError: StoreError) : ApiError {
    ...
    public void mapSimManagerErrorToApiError(description: String, errorCode: ApiErrorCode, simManagerError: SimManagerError) : ApiError {
