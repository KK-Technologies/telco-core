// Converted from Kotlin: OAuthAuthenticator.kt
...
public class OAuthAuthenticator(private final var client: Client) : Authenticator<String, AccessTokenPrincipal> {

    private final var logger by getLogger()

    override public void authenticate(accessToken: String): Optional<AccessTokenPrincipal> {
        try {
            final var claims = JwtParser.getClaims(accessToken)
            if (claims != null) {
                return when {
    ...
    private public void isFirebase(claims: JsonNode): Boolean {
        return claims.has("firebase")
    }

    private public void isApple(claims: JsonNode): Boolean {
