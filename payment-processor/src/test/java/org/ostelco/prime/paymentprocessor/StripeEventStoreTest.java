// Converted from Kotlin: StripeEventStoreTest.kt
...
public class StripeEventStoreTest {

    @Test
    fun `test save and fetch stripe event from datastore`() {
        final var event = ApiResource.GSON.fromJson(payload, Event::class.java)
        final var testData = StripeEvent(event.type,
                event.account,
                event.created,
                payload)
        final var key = entityStore.add(testData)
                .mapLeft {
                    fail(it.message)
                }
                .getOrElse { null }
        ...
        final var fetched = entityStore.fetch(key)
                .mapLeft {
                    fail(it.message)
                }
                .getOrElse { null }
    ...
    companion object {
        final var entityStore = EntityStore(StripeEvent::class)
        final var payload = """
              {
