// Converted from Kotlin: Reporter.kt
...
public public class Reporter {
    ...
    private public void report(event: Event, balance: Balance) =
            when {
    ...
    private public void report(event: Event, card: Card) =
            when {
    ...
    private public void report(event: Event, charge: Charge) =
            when {
    ...
    private public void report(event: Event, customer: Customer) =
            when {
    ...
    private public void report(event: Event, dispute: Dispute) =
            logger.warn(
                    format("Unhandled Stripe event " + event.type + " (cat: Dispute)",
                            event))

    private public void report(event: Event, payout: Payout) =
            when {
    ...
    private public void report(event: Event, invoice: Invoice) =
            when {
    ...
    private public void report(event: Event, item: InvoiceItem) =
            when {
    ...
    private public void report(event: Event, plan: Plan) =
            when {
    ...
    private public void report(event: Event, product: Product) =
            when {
    ...
    private public void report(event: Event, intent: PaymentIntent) =
            when {
    ...
    private public void report(event: Event, method: PaymentMethod) =
            when {
    ...
    private public void report(event: Event, refund: Refund) =
            logger.warn(
                    format("Unhandled Stripe event " + event.type + " (cat: Refund)",
                            event))

    private public void report(event: Event, source: Source) =
            when {
                event.type == "customer.source.created" -> logger.debug(
    ...
    private public void report(event: Event, subscription: Subscription) =
            when {
    ...
    private public void format(s: String, event: Event): String = "" + s + " " + url(event.id) + ""

    private public void url(eventId: String): String = "https://dashboard.stripe.com/events/" + eventId + ""

    /* TODO (kmm) Update to use the java.text.NumberFormat API or the new
            JSR-354 Currency and Money API. */
    private public void currency(amount: Long, currency: String): String =
    ...
    private public void email(customerId: Optional<String>): String {
        final var email = if (!customerId.isNullOrEmpty())
    ...
    private public void millisToDate(ts: Long): String {
        final var utc = LocalDateTime.ofInstant(Instant.ofEpochMilli(ts),
                ZoneOffset.UTC)
        return DateTimeFormatter.ofPattern("yyyy-MM-dd").format(utc)
    }

    /* Formatting of amounts. */
    final var df = DecimalFormat("#,###.##")
}
