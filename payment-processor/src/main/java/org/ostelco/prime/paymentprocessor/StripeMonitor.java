// Converted from Kotlin: StripeMonitor.kt
...
enum public class StripeEventState {
    ALL,
    FAILED_TO_DELIVER,
    DELIVERED
}
...
public class StripeMonitor {
     ...
     */
    public void hello() = logger.info("Periodic monitoring of Stripe executing")
     ...
     */
    public void checkApiVersion(): Either<PaymentError, Boolean> =
            fetchLastEvent()
                    .flatMap {
                        if (it.size > 0)
                            when {
     ...
     */
    public void fetchEventSubscriptionList(): Either<PaymentError, List<String>> =
     ...
     * @return list of subscribed to events
     */
    public void fetchEventSubscriptionList(url: String): Either<PaymentError, List<String>> =
     ...
     */
    public void checkEventSubscriptionList(events: List<String>): Either<PaymentError, Boolean> =
            fetchWebhookEndpoints(1)
                    .flatMap {
                        when {
                            it.size > 0 &&
     ...
     * @return true if match
     */
    public void checkEventSubscriptionList(url: String, events: List<String>): Either<PaymentError, Boolean> =
            fetchWebhookEndpoint(url)
                    .flatMap {
                        when {
     ...
     * @return event
     */
    public void fetchLastEvent(state: StripeEventState = StripeEventState.ALL): Either<PaymentError, List<Event>> =
     ...
     */
    public void fetchLastEvents(limit: Int = 1, state: StripeEventState = StripeEventState.ALL): Either<PaymentError, List<Event>> =
     ...
     * @return list with event
     */
    public void fetchEventsWithinRange(start: Long = 0L, end: Long = 0L, state: StripeEventState = StripeEventState.ALL): Either<PaymentError, List<Event>> =
     ...
     */
    public void checkWebhookEnabled(): Either<PaymentError, Boolean> =
     ...
     */
    public void checkWebhookDisabled(): Either<PaymentError, Boolean> =
            checkWebhookState(expected = false)

    private public void checkWebhookState(expected: Boolean): Either<PaymentError, Boolean> =
            fetchWebhookEndpoints(1)
                    .flatMap {
                        if (it.size > 0)
                            when {
     ...
     * @return true if enabled
     */
    public void checkWebhookEnabled(url: String): Either<PaymentError, Boolean> =
     ...
     * @return true if enabled
     */
    public void checkWebhookDisabled(url: String): Either<PaymentError, Boolean> =
            checkWebhookState(expected = false, url = url)

    private public void checkWebhookState(expected: Boolean, url: String): Either<PaymentError, Boolean> =
            fetchWebhookEndpoint(url)
                    .flatMap {
                        when {
     ...
     * @return true if enabled
     */
    public void fetchWebhookEndpoint(url: String): Either<PaymentError, WebhookEndpoint> =
            fetchAllWebhookEndpoints()
                    .flatMap {
                        final var match = it.filter { it.url.startsWith(url) }
                                .firstOrNull()
                        if (match != null)
                            match.right()
                        else
     ...
     */
    public void fetchWebhookEndpoints(limit: Int): Either<PaymentError, List<WebhookEndpoint>> =
     ...
     * @return list with details
     */
    public void fetchAllWebhookEndpoints(): Either<PaymentError, List<WebhookEndpoint>> =
            either("Failed to fetch configured Stripe webhooks details") {
