// Converted from Kotlin: StripePaymentProcessor.kt
...
public class StripePaymentProcessor : PaymentProcessor {
    ...
    override public void getSavedSources(customerId: String): Either<PaymentError, List<SourceDetailsInfo>> =
            either("Failed to retrieve sources for customer " + customerId + "") {
                final var customer = Customer.retrieve(customerId)
                final var sources: List<SourceDetailsInfo> = customer.sources.data.map {
                    final var details = getAccountDetails(it)
                    SourceDetailsInfo(it.id, getAccountType(details), details)
                }
                sources.sortedByDescending { it.details["created"] as Long }
            }

    private public void getAccountType(details: Map<String, Any>): String {
    ...
    private public void getAccountDetails(paymentSource: PaymentSource): Map<String, Any> =
            when (paymentSource) {
                is Card -> {
                    mapOf(
    ...
    private public void getCreatedTimestampFromMetadata(id: String, metadata: Map<String, Any>): Long {
        final var created: Optional<String> = metadata["created"] Optional<as> String
        return with (Optional<created>.toLongOrNull()) {
            if (this != null) {
    ...
    override public void createPaymentProfile(customerId: String, email: String): Either<PaymentError, ProfileInfo> =
            either("Failed to create profile for user " + customerId + "") {
                final var customerParams = mapOf(
                        "id" to customerId,
                        "email" to email,
    ...
    override public void getPaymentProfile(customerId: String): Either<PaymentError, ProfileInfo> =
    ...
    private public void getCustomer(customerId: String): Either<PaymentError, Customer> =
            Try {
    ...
    override public void createPlan(productId: String, amount: Int, currency: String, interval: PaymentProcessor.Interval, intervalCount: Long): Either<PaymentError, PlanInfo> =
            either("Failed to create plan for product " + productId + " amount " + amount + " currency " + currency + " interval " + interval.value + "") {
                final var planParams = mapOf(
                        "product" to productId,
                        "amount" to amount,
                        "interval" to interval.value,
                        "interval_count" to intervalCount,
                        "currency" to currency)
                final var plan = Plan.create(planParams)
                PlanInfo(plan.id)
            }

    override public void removePlan(planId: String): Either<PaymentError, PlanInfo> =
    ...
    override public void createProduct(name: String): Either<PaymentError, ProductInfo> =
            either("Failed to create product with name " + name + "") {
                final var productParams = mapOf(
                        "name" to name,
                        "type" to "service")
                ProductInfo(Product.create(productParams).id)
            }

    override public void removeProduct(productId: String): Either<PaymentError, ProductInfo> =
            either("Failed to delete product " + productId + "") {
                final var product = Product.retrieve(productId)
                ProductInfo(product.delete().id)
            }

    override public void addSource(customerId: String, sourceId: String): Either<PaymentError, SourceInfo> =
            either("Failed to add source " + sourceId + " to customer " + customerId + "") {
                final var customer = Customer.retrieve(customerId)
                final var sourceParams = mapOf("source" to sourceId,
    ...
    override public void setDefaultSource(customerId: String, sourceId: String): Either<PaymentError, SourceInfo> =
            either("Failed to set default source " + sourceId + " for customer " + customerId + "") {
                final var customer = Customer.retrieve(customerId)
                final var updateParams = mapOf("default_source" to sourceId)
                final var customerUpdated = customer.update(updateParams)
                SourceInfo(customerUpdated.defaultSource)
            }

    override public void getDefaultSource(customerId: String): Either<PaymentError, SourceInfo> =
            either("Failed to get default source for customer " + customerId + "") {
                SourceInfo(Customer.retrieve(customerId).defaultSource)
            }

    override public void removePaymentProfile(customerId: String): Either<PaymentError, ProfileInfo> =
            getCustomer(customerId)
                    .flatMap { customer ->
    ...
    override public void createSubscription(planId: String, customerId: String, trialEnd: Long, taxRegionId: Optional<String>): Either<PaymentError, SubscriptionDetailsInfo> =
            either("Failed to subscribe customer " + customerId + " to plan " + planId + "") {
                final var item = mapOf("plan" to planId)
                final var taxRates = getTaxRatesForTaxRegionId(taxRegionId)
                        .fold(
                                { emptyList<TaxRateInfo>() },
                                { it }
                        )
                final var subscriptionParams = mapOf(
                ...
                final var subscription = Subscription.create(subscriptionParams)
                final var status = subscriptionStatus(subscription)
    ...
    private public void subscriptionStatus(subscription: Subscription): Triple<PaymentStatus, String, String> {
        final var invoice = subscription.latestInvoiceObject
        final var intent = Optional<invoice>.paymentIntentObject
    ...
    override public void cancelSubscription(subscriptionId: String, invoiceNow: Boolean): Either<PaymentError, SubscriptionInfo> =
            either("Failed to unsubscribe subscription Id : " + subscriptionId + " with 'invoice-now' set to " + invoiceNow + "") {
                ...
                final var subscriptionParams = mapOf("invoice_now" to invoiceNow)
    ...
    override public void authorizeCharge(customerId: String, sourceId: Optional<String>, amount: Int, currency: String): Either<PaymentError, String> {
        final var errorMessage = "Failed to authorize the charge for customerId " + customerId + " sourceId " + sourceId + " amount " + amount + " currency " + currency + ""
        return when (amount) {
            0 -> "ZERO_CHARGE_" + UUID.randomUUID() + ""
                    .right()
            else -> either(errorMessage) {
                final var chargeParams = mutableMapOf(
                        "amount" to amount,
                        "currency" to currency,
                        "customer" to customerId,
                        "capture" to false)
                ...
                final var review = charge.review
                Either.cond(
                        test = (review == null),
                        ifTrue = { charge.id },
    ...
    override public void captureCharge(chargeId: String, customerId: String, amount: Int, currency: String): Either<PaymentError, String> {
        final var errorMessage = "Failed to capture charge for customerId " + customerId + " chargeId " + chargeId + ""
        return when (amount) {
            0 -> chargeId.right()
            else -> either(errorMessage) {
    ...
    override public void refundCharge(chargeId: String): Either<PaymentError, String> =
            either("Failed to refund charge " + chargeId + "") {
                final var refundParams = mapOf("charge" to chargeId)
                Refund.create(refundParams).id
            }

    override public void refundCharge(chargeId: String, amount: Int): Either<PaymentError, String> =
            when (amount) {
                0 -> chargeId.right()
                else -> either("Failed to refund charge " + chargeId + "") {
                    final var refundParams = mapOf(
                            "charge" to chargeId,
                            "amount" to amount)
                    Refund.create(refundParams).id
                }
    ...
    override public void removeSource(customerId: String, sourceId: String): Either<PaymentError, SourceInfo> =
            either("Failed to remove source " + sourceId + " for customerId " + customerId + "") {
                final var accountInfo = Customer.retrieve(customerId).sources.retrieve(sourceId)
                when (accountInfo) {
    ...
    override public void getStripeEphemeralKey(customerId: String, email: String, apiVersion: String): Either<PaymentError, String> =
    ...
    override public void createInvoiceItem(customerId: String, amount: Int, currency: String, description: String): Either<PaymentError, InvoiceItemInfo> =
            either("Failed to create an invoice item for customer " + customerId + " with Stripe") {
                final var params = mapOf(
                        "customer" to customerId,
                        "amount" to amount,
                        "currency" to currency,
    ...
    override public void removeInvoiceItem(invoiceItemId: String): Either<PaymentError, InvoiceItemInfo> =
            either("Failed to remove invoice item " + invoiceItemId + " with Stripe") {
                final var invoiceItem = InvoiceItem.retrieve(invoiceItemId).delete()
                InvoiceItemInfo(invoiceItem.id)
            }

    override public void createInvoice(customerId: String, taxRates: List<TaxRateInfo>, sourceId: Optional<String>): Either<PaymentError, InvoiceInfo> =
            createAndGetInvoiceDetails(customerId, taxRates, sourceId)
                    .flatMap {
                        InvoiceInfo(it.id).right()
                    }

    override public void createInvoice(customerId: String, amount: Int, currency: String, description: String, taxRegionId: Optional<String>, sourceId: Optional<String>): Either<PaymentError, InvoiceInfo> =
            createInvoiceItem(customerId, amount, currency, description)
                    ...
                    .flatMap { invoice ->
                        ...
                        final var items = invoice.lines.list(emptyMap()).data
                        ...
                        if (items.size > 1) {
                            ...
                            final var errorMessage = "Incorrect number of line items when attempting to create invoice " + invoice.id + " " +
    ...
    private public void createAndGetInvoiceDetails(customerId: String, taxRates: List<TaxRateInfo>, sourceId: Optional<String>): Either<PaymentError, Invoice> =
            either("") {
                final var params = mapOf(
                        "customer" to customerId,
    ...
    override public void payInvoice(invoiceId: String): Either<PaymentError, InvoicePaymentInfo> =
            either("Failed to complete payment of invoice " + invoiceId + "") {
                final var receipt = Invoice.retrieve(invoiceId).pay()
                InvoicePaymentInfo(receipt.id, Optional<receipt>.charge ?: UUID.randomUUID().toString())
            }

    override public void removeInvoice(invoiceId: String): Either<PaymentError, InvoiceInfo> =
            Try {
    ...
    private public void removeInvoice(invoice: Invoice): Either<PaymentError, InvoiceInfo> =
    ...
    companion object {
        final var TAX_REGION = "tax-region"
    }

    /* NOTE! This method creates a 'tax rate' with Stipe, unless there already
             exists a 'tax-rate' with the same information. */
    override public void createTaxRateForTaxRegionId(taxRegionId: String, percentage: BigDecimal, displayName: String, inclusive: Boolean): Either<PaymentError, TaxRateInfo> =
            getTaxRatesForTaxRegionId(taxRegionId)
                    .flatMap { taxRates ->
                        final var match = taxRates.find {
                            it.percentage == percentage && it.displayName == displayName && it.inclusive == inclusive
                        }

                        if (match == null) {
                            final var param = mapOf(
    ...
    override public void getTaxRatesForTaxRegionId(taxRegionId: Optional<String>): Either<PaymentError, List<TaxRateInfo>> =
            if (taxRegionId != null)
                getTaxRates()
                        .flatMap {
                            final var lst = it.filter { x ->
    ...
    private public void getTaxRates(): Either<PaymentError, List<TaxRate>> =
            either("Failed to fetch list with tax-rates from Stripe") {
                final var taxRateParameters = mapOf(
                        "active" to true
                )
    ...
    private public void ofEpochMilliToSecond(ts: Long): Long = ts.div(1000L)
}
