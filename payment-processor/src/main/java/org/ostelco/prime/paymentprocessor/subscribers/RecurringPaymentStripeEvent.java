// Converted from Kotlin: RecurringPaymentStripeEvent.kt
...
public class RecurringPaymentStripeEvent : PubSubSubscriber(
    ...
    override public void handler(message: ByteString, consumer: AckReplyConsumer) =
            Try {
                gson.fromJson(message.toStringUtf8(), Event::class.java)
                    ...
                    ifSuccess = { event ->
                        Try {
                            final var eventType = event.type
                            final var data = event.data.`object`  /* See comment in 'Reporter.kt'. */
    ...
    private public void invoiceEvent(eventType: String, invoice: Invoice) {
    ...
    private public void purchasedSubscriptionEvent(customerId: String, invoiceId: String, chargeId: String, plan: Plan) {
        final var productId = plan.product
        final var productDetails = Product.retrieve(productId)
