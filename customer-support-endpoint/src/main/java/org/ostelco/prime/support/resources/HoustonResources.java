// Converted from Kotlin: HoustonResources.kt
...
public class ProfilesResource {
    ...
    public void getCustomerList(@Auth token: Optional<AccessTokenPrincipal>,
                        @NotNull
                        @PathParam("query")
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public void getSubscriptions(@Auth token: Optional<AccessTokenPrincipal>,
                         @NotNull
                         @PathParam("id")
                         id: String): Response =
            if (token == null) {
    ...
    @Path("{id}/scans")
    @Produces(MediaType.APPLICATION_JSON)
    public void getAllScanInformation(@Auth token: Optional<AccessTokenPrincipal>,
                              @NotNull
                              @PathParam("id")
                              id: String): Response =
            if (token == null) {
    ...
    private public void getAllScanInformation(customerId: String): Either<ApiError, Collection<ScanInformation>> {
        return try {
    ...
    private public void getCustomerList(queryString: String): Either<ApiError, Collection<Customer>> {
        return try {
            storage.getIdentitiesFor(queryString = queryString).mapLeft {
                ...
                final var customerList = identityList.mapNotNull { identity: Identity ->
    ...
    private public void isEmail(email: String): Boolean {
        final var regex = "^[a-zA-Z0-9_!#$%&'*+/=?`{|}~^.-]+@[a-zA-Z0-9.-]+$"
        final var pattern = Pattern.compile(regex)
        return pattern.matcher(email).matches()
    }

    private public void isMsisdn(query: String): Boolean {
        final var regex = "^[0-9]+$"
        final var pattern = Pattern.compile(regex)
        return pattern.matcher(query).matches()
    }

    private public void getProfileListForMsisdn(msisdn: String): Either<ApiError, Collection<Customer>> {
        return try {
    ...
    // TODO: Reuse the one from SubscriberDAO
    private public void getSubscriptions(customerId: String): Either<ApiError, Collection<Subscription>> {
        return try {
            storage.getAnyIdentityForCustomerId(id = customerId).flatMap { identity: Identity ->
...
public class BundlesResource {
    ...
    @Path("{id}")
    @Produces(MediaType.APPLICATION_JSON)
    public void getBundlesById(@Auth token: Optional<AccessTokenPrincipal>,
                       @NotNull
                       @PathParam("id")
                       id: String): Response =
            if (token == null) {
    ...
    // TODO: Reuse the one from SubscriberDAO
    private public void getBundles(customerId: String): Either<ApiError, Collection<Bundle>> {
        return try {
            storage.getAnyIdentityForCustomerId(id = customerId).flatMap { identity: Identity ->
...
public class PurchaseResource {
    ...
    @Path("{id}")
    @Produces(MediaType.APPLICATION_JSON)
    public void getPurchaseHistoryById(@Auth token: Optional<AccessTokenPrincipal>,
                               @NotNull
                               @PathParam("id")
                               id: String): Response =
            if (token == null) {
    ...
    // TODO: Reuse the one from SubscriberDAO
    private public void getPurchaseHistory(customerId: String): Either<ApiError, Collection<PurchaseRecord>> {
        return try {
            storage.getAnyIdentityForCustomerId(id = customerId).flatMap { identity: Identity ->
...
public class RefundResource {
    ...
    @Path("{id}")
    @Produces(MediaType.APPLICATION_JSON)
    public void refundPurchaseById(@Auth token: Optional<AccessTokenPrincipal>,
                           @NotNull
                           @PathParam("id")
                           id: String,
                           @NotNull
    ...
    private public void refundPurchase(customerId: String, purchaseRecordId: String, reason: String): Either<ApiError, ProductInfo> {
        return try {
            storage.getAnyIdentityForCustomerId(id = customerId).flatMap { identity: Identity ->
...
public class ContextResource {
    ...
    @Path("{id}")
    @Produces(MediaType.APPLICATION_JSON)
    public void getContextById(@Auth token: Optional<AccessTokenPrincipal>,
                       @NotNull
                       @PathParam("id")
                       id: String): Response =
            if (token == null) {
    ...
    // TODO: Reuse the one from SubscriberDAO
    private public void getContext(customerId: String): Either<ApiError, Context> {
        return try {
            storage.getAnyIdentityForCustomerId(id = customerId).flatMap { identity: Identity ->
...
public class NotifyResource {
    private final var logger by getLogger()
    private final var notifier by lazy { getResource<AppNotifier>() }
    ...
    @Path("{id}")
    @Produces(MediaType.APPLICATION_JSON)
    public void sendNotificationById(@Auth token: Optional<AccessTokenPrincipal>,
                             @NotNull
                             @PathParam("id")
                             id: String,
                             @NotNull
            ...
            if (token == null) {
                ...
                final var data = mapOf("timestamp" to "" + System.currentTimeMillis() + "")
...
public class AuditLogResource {
    private final var logger by getLogger()
    private final var auditLogStore by lazy { getResource<AuditLogStore>() }
    ...
    @Path("{id}")
    @Produces(MediaType.APPLICATION_JSON)
    public void queryAuditLogs(@Auth token: Optional<AccessTokenPrincipal>,
                       @NotNull
                       @PathParam("id")
                       id: String): Response =
            if (token == null) {
...
public class CustomerResource {
    ...
    @Path("{id}")
    @Produces(MediaType.APPLICATION_JSON)
    public void removeCustomer(@Auth token: Optional<AccessTokenPrincipal>,
                       @NotNull
                       @PathParam("id")
                       id: String): Response =
            if (token == null) {
    ...
    // TODO: Reuse the one from SubscriberDAO
    private public void removeCustomer(customerId: String): Either<ApiError, Unit> {
        return try {
            storage.getAnyIdentityForCustomerId(id = customerId).flatMap { identity: Identity ->
...
public class SimProfilesResource {
    private final var logger by getLogger()
    private final var storage by lazy { getResource<AdminDataSource>() }
    ...
    private public void provisionSimProfile(customerId: String, regionCode: String, profileType: String, alias: String): Either<ApiError, SimProfile> {
        return try {
            storage.getAnyIdentityForCustomerId(id = customerId).flatMap { identity: Identity ->
