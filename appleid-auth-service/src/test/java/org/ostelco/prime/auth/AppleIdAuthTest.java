// Converted from Kotlin: AppleIdAuthTest.kt
...
public class AppleIdAuthTest {

    @Ignore
    @Test
    fun `test - Apple ID Auth test`() {

        final var loginToken = resources.target("/appleId/authorize")
                .request()
                .method("POST", Entity.json("""{"authCode":"" + AUTH_CODE + ""}"""), object : GenericType<LoginToken>() {})
        // """{"token":"ACCESS_TOKEN"}"""

        final var parts = loginToken.token.split('.')
        assertEquals(3, parts.size)
        assertEquals("""{"alg":"RS256"}""", String(Base64.getDecoder().decode(parts[0])))
        final var map: Map<String, *> = objectMapper.readValue(
    ...
    companion object {
        ...
        @JvmField
        @ClassRule
        final var dropwizardClientRule = DropwizardClientRule(AppleIdAuthServiceEmulator())
