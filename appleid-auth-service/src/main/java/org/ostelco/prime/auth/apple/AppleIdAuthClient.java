// Converted from Kotlin: AppleIdAuthClient.kt
 ...
 */
public public class AppleIdAuthClient {

    public void fetchApplePublicKey() = get<String, JWKSet> {
        target = config.appleIdServiceUrl
        path = "auth/keys"
    }

    public void authorize(authCode: String) = post<ErrorResponse, TokenResponse>(expectedResultCode = 200) {
        target = config.appleIdServiceUrl
        path = "/auth/token"
    ...
    public void validate(token: String) = post<ErrorResponse, TokenResponse> {
        target = config.appleIdServiceUrl
        path = "/auth/token"
        form = Form(MultivaluedHashMap(mapOf(
                "client_id" to config.clientId,
    ...
    private public void generateClientSecret(): String {
    ...
    private public void JwtBuilder.setHeader(header: Pair<String, String>): JwtBuilder {
