// Converted from Kotlin: PeriodicProvisioningTask.kt
...
public class PreallocateProfilesTask(
        private final var lowWaterMark: Int = 10,
        final var maxNoOfProfileToAllocate: Int = 30,
        final var simInventoryDAO: SimInventoryDAO,
        final var httpClient: CloseableHttpClient,
        final var hssAdapterProxy: SimManagerToHssDispatcherAdapter,
        final var profileVendors: List<ProfileVendorConfig>) : Task("preallocate_sim_profiles") {
    ...
    override public void execute(parameters: ImmutableMultimap<String, String>, output: PrintWriter) {
    ...
    private public void getConfigForVendorNamed(name: String) =
            profileVendors.firstOrNull {
                it.name == name
            }

    private public void getProfileVendorAdapterForProfileVendorId(profileVendorId: Long): Either<SimManagerError, ProfileVendorAdapter> =
            simInventoryDAO.getProfileVendorAdapterDatumById(profileVendorId)
                    .flatMap { datum ->
                        final var profileVendorConfig = getConfigForVendorNamed(datum.name)
    ...
    private public void preProvisionSimProfile(hssEntry: HssEntry,
    ...
    private public void batchPreprovisionSimProfiles(hssEntry: HssEntry,
        ...
        final var noOfProfilesToActuallyAllocate =
        ...
        if (noOfProfilesToActuallyAllocate == 0L) {
            ...
            for (i in 1..noOfProfilesToActuallyAllocate) {
                ...
                final var focus = simInventoryDAO.findNextNonProvisionedSimProfileForHss(hssId = hssEntry.id, profile = simProfileName)
                if (focus.isLeft()) {
                    return focus
                }
    ...
    private public void preProvisionSimProfile(simEntry: SimEntry, simProfileName: String, hssEntry: HssEntry): Either<SimManagerError, Any> {
    ...
    public void preAllocateSimProfiles(): Either<SimManagerError, Unit> =
    ...
    private public void preAllocateSimProfilesForHss(hssEntry: HssEntry): Either<SimManagerError, Unit> {
    ...
    private public void preAllocateSimProfileForHss(hssEntry: HssEntry, simProfileName: String): Either<SimManagerError, Unit> {
        final var thisProfilesNameForLogging = "profiles  where  ID/metricName/simProfileName=" + hssEntry.id + "/" + hssEntry.name + "/" + simProfileName + ""
        ...
        return simInventoryDAO.getProfileStats(hssEntry.id, simProfileName).flatMap { profileStats ->
            if (profileStats.noOfUnallocatedEntries == 0L) {
                final var msg = "No  more unallocated  " + thisProfilesNameForLogging + ""
                logger.error(msg)
                ...
                // To satisfy  Arrow we must all unite, Optional<right>, ....
                Unit.right()
            }
        }
    }
