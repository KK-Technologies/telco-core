// Converted from Kotlin: SimInventoryCallbackService.kt
 ...
 */
public class SimInventoryCallbackService(final var dao: SimInventoryDAO) : SmDpPlusCallbackService {

    private final var logger by getLogger()

    override public void handleDownloadProgressInfo(header: ES2RequestHeader,
                                            eid: Optional<String>,
                                            iccid: String,
                                            profileType: String,
                                            timestamp: String,
        ...
        final var numericIccId = iccid.toUpperCase().trimEnd('F')

        // If we can't find the ICCID, then cry foul and log an error message
        // that will get the ops team's attention asap!
        final var profileQueryResult = dao.getSimProfileByIccid(numericIccId)
        ...
        if (notificationPointStatus.status == FunctionExecutionStatusType.ExecutedSuccess) {
             ...
             *      be picked up by the state as stored in the database, and if the user public interface is dependent
             *      on that state, the user public interface may suffer a failure.  These issues needs to be gamed out
     ...
     *      errors are not si
     */
    private public void gotoState(iccid: String, targetSmdpPlusStatus: SmDpPlusState) {
