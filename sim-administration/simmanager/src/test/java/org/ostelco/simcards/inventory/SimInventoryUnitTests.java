// Converted from Kotlin: SimInventoryUnitTests.kt
...
public class SimInventoryUnitTests {

    companion object {
        // TODO why mock data/pojo Optional<classes>
        private final var config = mock(SimAdministrationConfiguration::class.java)
        private final var hssConfig = mock(SwtHssConfig::class.java)
        private final var profileVendorConfig = mock(ProfileVendorConfig::class.java)

        private final var dao = mock(SimInventoryDAO::class.java)
        private final var hssEntry = mock(HssEntry::class.java)
        private final var profileVendorAdapterDatum = mock(ProfileVendorAdapterDatum::class.java)
        private final var httpClient = mock(CloseableHttpClient::class.java)
    ...
    private final var fakeIccid1 = "01234567891234567890"
    private final var fakeIccid2 = "01234567891234567891"
    private final var fakeImsi1 = "12345678912345"
    private final var fakeImsi2 = "12345678912346"
    private final var fakeMsisdn1 = "474747474747"
    private final var fakeMsisdn2 = "464646464646"
    private final var fakeEid = "01010101010101010101010101010101"

    private final var fakeProfileVendor = "Foo"
    private final var fakeHlr = "Bar"
    private final var fakePhoneType = "_"
    private final var fakeProfile = "PROFILE_1"

    private final var matchingId = UUID.randomUUID().toString()
    private final var es9plusEndpoint = "http://localhost:8080/farFarAway"

    private final var fakeSimEntryWithoutMsisdn = SimEntry(
            id = 1L,
            profileVendorId = 1L,
            hssId = 1L,
            msisdn = "",
    ...
    private final var fakeSimEntryWithMsisdn = fakeSimEntryWithoutMsisdn.copy(
            msisdn = fakeMsisdn1,
            eid = fakeEid,
    ...
    @Test
    public void testFindByIccidPositiveResult() {
        final var response = RULE.target("/ostelco/sim-inventory/" + fakeHlr + "/iccid/" + fakeIccid1 + "")
    ...
    @Test
    public void testFindByIccidNegativeResult() {
        final var response = RULE.target("/ostelco/sim-inventory/" + fakeHlr + "/iccid/" + fakeIccid2 + "")
                .request(MediaType.APPLICATION_JSON)
                .get()
    ...
    @Test
    public void testFindByImsiPositiveResult() {
        final var response = RULE.target("/ostelco/sim-inventory/" + fakeHlr + "/imsi/" + fakeImsi1 + "")
                .request(MediaType.APPLICATION_JSON)
                .get()
        assertEquals(200, response.status)
    ...
    @Test
    public void testFindByImsiNegativeResult() {
        final var response = RULE.target("/ostelco/sim-inventory/" + fakeHlr + "/imsi/" + fakeImsi2 + "")
                .request(MediaType.APPLICATION_JSON)
                .get()
        assertEquals(404, response.status)
    ...
    @Test
    public void testFindByMsisdnPositiveResult() {
        final var response = RULE.target("/ostelco/sim-inventory/" + fakeHlr + "/msisdn/" + fakeMsisdn1 + "")
                .request(MediaType.APPLICATION_JSON)
                .get()
        assertEquals(200, response.status)
    ...
    @Test
    public void testFindByMsisdnNegativeResult() {
        final var response = RULE.target("/ostelco/sim-inventory/" + fakeHlr + "/msisdn/" + fakeMsisdn2 + "")
                .request(MediaType.APPLICATION_JSON)
                .get()
        assertEquals(404, response.status)
    ...
    @Test
    public void testActivateEsim() {
        final var response = RULE.target("/ostelco/sim-inventory/" + fakeHlr + "/esim")
                .request(MediaType.APPLICATION_JSON)
                .get()
        assertEquals(200, response.status)
    ...
    private public void importSimBatch(hssState: Optional<HssState> = null): SimImportBatch {
        final var sampleCsvIinput = """
        ...
        final var urlPath = "/ostelco/sim-inventory/" + fakeHlr + "/import-batch/profilevendor/" + fakeProfileVendor + ""
    ...
    private public void setUpMocksForMockedOutImportSims(initialHssState:HssState = HssState.NOT_ACTIVATED) {
    ...
    @Test
    public void testMockedOutImportSims() {
        setUpMocksForMockedOutImportSims()
        importSimBatch()
    }

    @Test
    public void testMockedOutImportSimsWithNonActicatedHssStatusSet() {
        setUpMocksForMockedOutImportSims()
        importSimBatch(HssState.NOT_ACTIVATED)
    }

    @Test
    public void testMockedOutImportSimsWithActivatedHssStatusSet() {
