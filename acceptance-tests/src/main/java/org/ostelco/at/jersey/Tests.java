...
public class CustomerTest {
    ...
    fun `jersey test - encoded email GET and PUT customer`() {
        ...
        final var email = "customer-" + randomInt() + "+test@test.com"
        final var nickname = "Test Customer"
        var customerId = ""
        try {
            final var createdCustomer: Customer = post {
                path = "/customer"
                queryParams = mapOf(
            ...
            final var customer: Customer = get {
                path = "/customer"
                this.email = email
            }
            ...
            final var newName = "New name: Test Customer"
            final var email2 = "customer-" + randomInt() + ".abc+test@test.com"

            final var updatedCustomer: Customer = put {
                path = "/customer"
                queryParams = mapOf(
    ...
    fun `jersey test - POST application token`() {
        ...
        try {
            ...
            final var reply: ApplicationToken = post {
...
public class RegionsTest {
    ...
    fun `jersey test - GET regions - No regions`() {
        ...
        try {
            ...
            final var regionDetailsList: Collection<RegionDetails> = get {
                path = "/regions"
                this.email = email
            }
    ...
    fun `jersey test - GET regions - Single Region with no profiles`() {
        ...
        try {
            ...
            final var regionDetailsList: RegionDetailsList = get {
                path = "/regions"
                this.email = email
            }

            final var naRegionDetails = regionDetailsList.singleOrNull { it.status != AVAILABLE }
            assertTrue(naRegionDetails != null, "List should contain only one region in a state other than available")

            final var noRegionDetails = regionDetailsList.singleOrNull { it.region.id == "no" }
    ...
    fun `jersey test - POST support simprofile - Sim profile from houston`() {
        ...
        try {
            ...
            final var regionCode = receivedRegion.region.id
            final var simProfile = post<SimProfile> {
...
public class SubscriptionsTest {
    ...
    fun `jersey test - GET subscriptions`() {
        ...
        try {
            ...
            final var subscriptions: Collection<Subscription> = get {
...
public class BundlesAndPurchasesTest {
    ...
    fun `jersey test - GET bundles`() {
        ...
        try {
            ...
            final var purchaseRecords: PurchaseRecordList = get {
                path = "/purchases"
                this.email = email
            }
...
public class GetProductsTest {
    ...
    fun `jersey test - GET products`() {
        ...
        try {
            ...
            final var products: List<Product> = get {
                path = "/products"
                this.email = email
            }
...
public class SourceTest {
    ...
    fun `jersey test - POST source create`() {
        ...
        try {
            ...
            final var sources: PaymentSourceList = get {
                path = "/paymentSources"
                this.email = email
            }
    ...
    fun `jersey test - GET list sources`() {
        ...
        try {
            ...
            final var createdIds = listOf(createTokenWithStripe(email),
    ...
    fun `jersey test - DELETE source`() {
        ...
        try {
            ...
            final var createdIds = listOf(getCardIdForTokenFromStripe(createTokenWithStripe(email)),
                    createSourceWithStripe(email))

            final var deletedIds = createdIds.map { removeSourceWithStripe(email, it) }
    ...
    private public void createTokenWithStripe(email: String): String {
        final var tokenId = StripePayment.createPaymentTokenId()
    ...
    private public void createSourceWithStripe(email: String): String {
    ...
    private public void removeSourceWithStripe(email: String, sourceId: String): String {
        final var removedSource = delete<PaymentSource> {
            path = "/paymentSources"
            this.email = email
            queryParams = mapOf("sourceId" to sourceId)
        }
...
public class PurchaseTest {
    ...
    fun `jersey test - POST products purchase`() {
        ...
        try {
            ...
            final var balanceBefore = get<List<Bundle>> {
                path = "/bundles"
                this.email = email
            }.first().balance
            ...
            final var balanceAfter = get<List<Bundle>> {
                path = "/bundles"
                this.email = email
            }.first().balance
    ...
    fun `jersey test - POST products purchase using default source`() {
        ...
        try {
            ...
            final var paymentSource: PaymentSource = post {
                path = "/paymentSources"
                this.email = email
                queryParams = mapOf("sourceId" to sourceId)
            }
    ...
    fun `jersey test - Refund purchase using default source`() {
        ...
        try {
            ...
            final var refundedProduct = put<ProductInfo> {
    ...
    fun `jersey test - POST products purchase add source then pay with it`() {
        ...
        try {
            ...
            final var bundlesBefore: BundleList = get {
                path = "/bundles"
                this.email = email
            }
            final var balanceBefore = bundlesBefore[0].balance
            ...
            final var bundlesAfter: BundleList = get {
                path = "/bundles"
                this.email = email
            }
            final var balanceAfter = bundlesAfter[0].balance
...
public class JumioKycTest {

    private final var imgUrl = "https://www.gstatic.com/webp/gallery3/1.png"
    private final var imgUrl2 = "https://www.gstatic.com/webp/gallery3/2.png"

    @Test
    fun `jersey test - GET new-ekyc-scanId - generate new scanId for eKYC`() {

        final var email = "ekyc-" + randomInt() + "@test.com"
        var customerId = ""
        try {
            customerId = createCustomer(name = "Test User for eKYC", email = email).id

            final var scanInfo: ScanInformation = post {
                path = "regions/no/kyc/jumio/scans"
                this.email = email
            }
            assertNotNull(scanInfo.scanId, message = "Failed to get new scanId")

            final var regionDetailsList = get<RegionDetailsList> {
                path = "/regions"
                this.email = email
            }
    ...
    fun `jersey test - ekyc callback - process failure of face id`() {
        ...
        try {
            ...
            dataMap["idDob"] = listOf("1990-12-09")
            dataMap["merchantIdScanReference"] = listOf(scanInfo.scanId)
            final var identityVerification = """{ "similarity":"MATCH", "validity":"FALSE", "reason": "ENTIRE_ID_USED_AS_SELFIE" }"""
            dataMap["identityVerification"] = listOf(identityVerification)
    ...
    fun `jersey test - ekyc callback - reject & approve`() {
        ...
        try {
            ...
            final var newScanInfo: ScanInformation = post {
                path = "/regions/no/kyc/jumio/scans"
                this.email = email
            }
            ...
            final var dataMap2 = MultivaluedHashMap<String, String>()
            ...
            final var newRegionDetailsList = get<RegionDetailsList> {
                path = "/regions"
                this.email = email
            }
    ...
    fun `jersey test - ekyc verify 2 scans`() {
        ...
        try {
            ...
            final var regionDetailsList1 = get<RegionDetailsList> {
                path = "/regions"
                this.email = email
            }
            ...
            final var stringList = "[ \"" + imgUrl + "\", \"" + imgUrl2 + "\" ]"
            ...
            final var regionDetailsList2 = get<RegionDetailsList> {
                path = "/regions"
                this.email = email
            }
    ...
    fun `jersey test - ekyc rejected with detailed reject reason`() {
        ...
        try {
            ...
            assertNotNull(scanInfo.scanId, message = "Failed to get new scanId")
            final var rejectReason = """{ "rejectReasonCode":"100", "rejectReasonDescription":"MANIPULATED_DOCUMENT", "rejectReasonDetails": [{ "detailsCode": "1001", "detailsDescription": "PHOTO" },{ "detailsCode": "1004", "detailsDescription": "DOB" }]}"""
...
public class SingaporeKycTest {
    ...
    fun `jersey test - GET myinfoConfig v3`() {
        ...
        try {
            ...
            final var myInfoConfig = get<MyInfoConfig> {
    ...
    fun `jersey test - GET myinfo v3`() {
        ...
        try {
            ...
            final var personData: String = get {
            ...
            run {
                ...
                final var newRegionDetailsList = regionDetailsList.singleOrNull { it.region.id == "sg" }
    ...
    fun `jersey test - NRIC, Jumio and address`() {
        ...
        try {
            ...
            run {
                ...
                final var sgRegionDetails = regionDetailsList.singleOrNull { it.region.id == "sg" }
...
{"description":"Incomplete customer description. Subscriber - " + email + " not found."} expected:<200> but was:<404>
    ...
    fun `jersey test - POST customer`() {
        ...
        try {
            ...
            // for first
            final var referralsForFirst: List<Person> = get {
                path = "/referred"
                email = firstEmail
            }
            assertEquals(listOf("Test Referral Second User"), referralsForFirst.map { it.name })

            final var referredByForFirst: Person = get {
                path = "/referred/by"
                email = firstEmail
            }
            ...
            // for referred_by_foo
            final var referralsForSecond: List<Person> = get {
                path = "/referred"
                email = secondEmail
            }
            assertEquals(emptyList(), referralsForSecond.map { it.name })

            final var referredByForSecond: Person = get {
                path = "/referred/by"
                email = secondEmail
            }
            assertEquals("Test Referral First User", referredByForSecond.name)

            final var secondSubscriberBundles: BundleList = get {
                path = "/bundles"
                email = secondEmail
            }
            ...
            final var secondSubscriberPurchases: PurchaseRecordList = get {
                path = "/purchases"
                email = secondEmail
            }
...
public class GraphQlTests {
    ...
    fun `jersey test - POST graphql`() {
        ...
        try {
            ...
            final var response = post<GraphQlResponse>(expectedResultCode = 200) {
                path = "/graphql"
                this.email = email
                body = mapOf("query" to graphqlPostQuery)
            }

            final var context = response.Optional<data>.context

            assertEquals(expected = "Test GraphQL POST Endpoint", actual = Optional<context>.Optional<customer>.nickname)
            assertEquals(expected = email, actual = Optional<context>.Optional<customer>.contactEmail)
            assertEquals(expected = 2_147_483_648L, actual = Optional<context>.Optional<bundles>.first()?.balance)
            assertEquals(expected = "no", actual = Optional<context>.Optional<regions>.first()?.Optional<region>.id)
            assertEquals(expected = "Norway", actual = Optional<context>.Optional<regions>.first()?.Optional<region>.name)
            assertEquals(expected = APPROVED, actual = Optional<context>.Optional<regions>.first()?.status)
            assertEquals(
                    expected = mapOf(KycType.JUMIO.name to KycStatus.APPROVED),
                    actual = Optional<context>.Optional<regions>.first()?.Optional<kycStatusMap>.filterValues { it != null }
            )
            assertEquals(expected = "TEST-unknown", actual = Optional<context>.Optional<regions>.first()?.Optional<simProfiles>.first()?.iccId)
            assertEquals(expected = "Dummy eSIM", actual = Optional<context>.Optional<regions>.first()?.Optional<simProfiles>.first()?.eSimActivationCode)
            assertEquals(expected = "default", actual = Optional<context>.Optional<regions>.first()?.Optional<simProfiles>.first()?.alias)
            assertEquals(expected = SimProfile.StatusEnum.INSTALLED, actual = Optional<context>.Optional<regions>.first()?.Optional<simProfiles>.first()?.status)
        } finally {
            StripePayment.deleteCustomer(customerId = customerId)
        }
    }
    ...
    fun `jersey test - GET graphql`() {
        ...
        try {
            ...
            final var response = get<GraphQlResponse> {
                path = "/graphql"
                this.email = email
            ...
            assertEquals(expected = "Test GraphQL GET Endpoint", actual = Optional<context>.Optional<customer>.nickname)
            assertEquals(expected = email, actual = Optional<context>.Optional<customer>.contactEmail)
            assertEquals(expected = 2_147_483_648L, actual = Optional<context>.Optional<bundles>.first()?.balance)
            assertEquals(expected = "no", actual = Optional<context>.Optional<regions>.first()?.Optional<region>.id)
            assertEquals(expected = "Norway", actual = Optional<context>.Optional<regions>.first()?.Optional<region>.name)
