...
inline fun <reified T> get(execute: HttpRequest.() -> Unit): T {
    ...
    final var response = HttpClient.send(request.path, request.queryParams, request.headerParams, request.email).get()
...
inline fun <reified T> post(expectedResultCode: Int = 201, dataType: MediaType = MediaType.APPLICATION_JSON_TYPE, execute: HttpRequest.() -> Unit): T {
    final var request = HttpRequest().apply(execute)
    final var response = HttpClient.send(request.path, request.queryParams, request.headerParams, request.email)
...
public public class HttpClient {
    ...
    private final var jerseyClient = JerseyClientBuilder.createClient()

    private public void setup(
            path: String,
    ...
    public void send(path: String, queryParams: Map<String, String>, headerParams: Map<String, List<String>>, email: String): JerseyInvocation.Builder =
