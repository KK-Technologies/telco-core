// Converted from Kotlin: AvpTypeDictionary.kt
...
public public class AvpTypeDictionary {

    private final var logger by getLogger()

    private final var avpRepMap: MutableMap<Int, Optional<AvpType>> = HashMap()
    private final var avpTypeMap: MutableMap<String, Optional<AvpType>> = HashMap()
    ...
    private public void dictionaryPath(): String {
        var configPath: Optional<String> = System.getenv("CONFIG_FOLDER")
        if (configPath == null) {
            configPath = "config"
        }
    ...
    public void getType(avp: Avp): Optional<AvpType> {

        var avpType: Optional<AvpType> = avpRepMap[avp.code]
        ...
        if (avpType == null) {
            ...
            var avpRep: Optional<AvpRepresentation> = AvpDictionary.INSTANCE.getAvp(avp.code, avp.vendorId)
...
enum public class AvpType(final var label: String) {
    ADDRESS("Address"),
    IDENTITY("Identity"),
    URI("URI"),
    FLOAT32("Float32"),
...
enum public class AvpRep(final var avpCode: Int, final var avpType: AvpType) {
