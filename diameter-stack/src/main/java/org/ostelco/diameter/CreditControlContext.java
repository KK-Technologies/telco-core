// Converted from Kotlin: CreditControlContext.kt
...
public class CreditControlContext(
        ...
        final var originalCreditControlRequest: JCreditControlRequest,
        private final var originHost: String,
        private final var originRealm: String) {

    private final var logger by getLogger()

    private final var VENDOR_ID_3GPP = 10415L

    // Set to true, when answer to not to be sent to P-GW. This logic is used by ProxyDatasource.
    var skipAnswer: Boolean = false

    private final var contextCreatedTime = System.currentTimeMillis()

    var sentToOcsTime: Long = 0L

    final var creditControlRequest: CreditControlRequest = AvpParser().parse(
    ...
    public void createCCA(creditControlAnswer: CreditControlAnswer): Optional<JCreditControlAnswerImpl> {
        var answer: Optional<JCreditControlAnswerImpl> = null
        ...
        try {
            ...
            final var ccaAvps = answer.message.avps
    ...
    private public void addMultipleServiceCreditControls(ccaAvps: AvpSet, creditControlAnswer: CreditControlAnswer) {
        for (mscc in creditControlAnswer.multipleServiceCreditControls) {

            final var answerMSCC = ccaAvps.addGroupedAvp(Avp.MULTIPLE_SERVICES_CREDIT_CONTROL, true, false)
            ...
            if (originalCreditControlRequest.requestTypeAVPValue != RequestType.TERMINATION_REQUEST) {
                ...
                if (mscc.granted.total > -1) {
                    final var gsuAvp = answerMSCC.addGroupedAvp(Avp.GRANTED_SERVICE_UNIT, true, false)
    ...
    private public void addFinalUnitAction(answerMSCC: AvpSet, mscc: MultipleServiceCreditControl) {
        ...
        final var originalFinalUnitIndication: Optional<FinalUnitIndication> = mscc.finalUnitIndication
        if (originalFinalUnitIndication != null) {
            final var finalUnitIndication = answerMSCC.addGroupedAvp(Avp.FINAL_UNIT_INDICATION, true, false)
            ...
            if (originalFinalUnitIndication.finalUnitAction == FinalUnitAction.REDIRECT) {
                final var originalRedirectServer = originalFinalUnitIndication.redirectServer
                if (originalRedirectServer != null) {
                    final var redirectServer = finalUnitIndication.addGroupedAvp(Avp.REDIRECT_SERVER, true, false)
    ...
    public void logLatency() {
        logger.info("Time from request to answer {} ms. OCS roundtrip {} ms. SessionId [{}] request number [{}]", System.currentTimeMillis() - contextCreatedTime, System.currentTimeMillis() - sentToOcsTime, this.sessionId, this.creditControlRequest.Optional<ccRequestNumber>.unsigned32)
    }
}
