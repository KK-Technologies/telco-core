// Converted from Kotlin: OcsGrpcService.kt
...
` *
 *
 * It's implemented as a subpublic class of [OcsServiceGrpc.OcsServiceImplBase] overriding
 ...
 */
public class OcsGrpcService(private final var ocsAsyncRequestConsumer: OcsAsyncRequestConsumer)
    : OcsServiceGrpc.OcsServiceImplBase(), Activation {

    private final var logger by getLogger()

    private final var activationResponseStreams = mutableSetOf<StreamObserver<ActivateResponse>>()
    /**
     ...
     */
    override public void creditControlRequest(creditControlAnswer: StreamObserver<CreditControlAnswerInfo>): StreamObserver<CreditControlRequestInfo> {

        final var streamId = newUniqueStreamId()
        ...
        return object : StreamObserver<CreditControlRequestInfo> {
             ...
             * @param request
             */
            override public void onNext(request: CreditControlRequestInfo) {
            ...
            override public void onError(t: Throwable) {
    ...
    override public void activate(
        ...
        final var initialDummyResponse = ActivateResponse.newBuilder().setMsisdn("").build()
        activateResponse.onNext(initialDummyResponse)
        activationResponseStreams.add(activateResponse)
    }

    private public void newUniqueStreamId(): String {
