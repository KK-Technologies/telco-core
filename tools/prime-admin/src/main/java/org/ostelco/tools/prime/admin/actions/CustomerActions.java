// Converted from Kotlin: CustomerActions.kt
...
public void createCustomer(email: String, nickname: String): Either<StoreError, Unit> = adminStore
        .addCustomer(
...
public void approveRegionForCustomer(email: String, regionCode: String): Either<StoreError, Unit> = Either.fx {

    final var customerId = adminStore
...
public void addCustomerToSegment(email: String, segmentId: String): Either<StoreError, Unit> = writeTransaction {
    Either.fx {
        final var customerId = adminStore
                .getCustomer(identity = emailAsIdentity(email = email))
                .bind()
...
public void deleteCustomer(email: String) = adminStore
...
public void setBalance(email: String, balance: Long) = adminStore
...
public void getAllRegionDetails(email: String) = adminStore
        .getAllRegionDetails(identity = emailAsIdentity(email = email))

public void getRegionDetails(email: String, regionCode: String) = adminStore
        .getRegionDetails(
                identity = emailAsIdentity(email = email),
                regionCode = regionCode
        )
...
// Common
private public void emailAsIdentity(email: String) = Identity(
        id = email,
        type = "EMAIL",
        provider = "email"
)

public void identifyCustomer(idDigests: Set<String>) {
