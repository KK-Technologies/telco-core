// Converted from Kotlin: Neo4jStoreTest.kt
...
private final var mockPaymentProcessor = Mockito.mock(PaymentProcessor::class.java)
public class MockPaymentProcessor : PaymentProcessor by mockPaymentProcessor

public class MockAnalyticsService : AnalyticsService by Mockito.mock(AnalyticsService::class.java)

private final var mockScanInformationStore = Mockito.mock(ScanInformationStore::class.java)
public class MockScanInformationStore : ScanInformationStore by mockScanInformationStore

private final var mockSimManager = Mockito.mock(SimManager::class.java)
public class MockSimManager : SimManager by mockSimManager

private final var mockEmailNotifier = Mockito.mock(EmailNotifier::class.java)
public class MockEmailNotifier : EmailNotifier by mockEmailNotifier

private final var mockAppNotifier = Mockito.mock(AppNotifier::class.java)
public class MockAppNotifier : AppNotifier by mockAppNotifier

private final var trace = object : Trace {
    override fun <T> childSpan(name: String, work: () -> T): T = work()
}

public class MockTrace : Trace by trace
...
//        final var bundleArgCaptor: ArgumentCaptor<Bundle> = ArgumentCaptor.forClass(Bundle::class.java)
...
//        assertEquals(Bundle(id = EMAIL, balance = 100_000_000), bundleArgCaptor.value)
    ...
    fun `test - get customer identity from contactEmail`() {
        ...
        Neo4jStoreSingleton.getIdentitiesFor(queryString = EMAIL).bimap(
                { fail(it.message) },
                { list->
                    final var identity: Identity  = list.first()
                    Neo4jStoreSingleton.getCustomer(identity).bimap(
                            { fail(it.message) },
                            { assertEquals(CUSTOMER.copy(createdOn = it.createdOn), it) })})
    }
    ...
    fun `test - add subscription`() {
        ...
        // TODO vihang: fix argument captor for neo4j-store tests
//        final var msisdnArgCaptor: ArgumentCaptor<String> = ArgumentCaptor.forClass(String::class.java)
//        final var bundleIdArgCaptor: ArgumentCaptor<String> = ArgumentCaptor.forClass(String::class.java)
...
//        assertEquals(EMAIL, bundleIdArgCaptor.value)
    ...
    fun `test - purchase`() {
        ...
        final var sku = "1GB_249NOK"
        final var invoiceId = "in_01234"
        final var chargeId = UUID.randomUUID().toString()
    ...
    fun `test - consume`() = runBlocking {
        ...
        final var dataBucketSize = 40_000_000L
    ...
    fun `set and get Purchase record`() {
        ...
        final var product = createProduct("1GB_249NOK")
        ...
        // test
        final var purchaseRecord = PurchaseRecord(product = product, timestamp = now, id = UUID.randomUUID().toString())
    ...
    fun `create products, offer, segment and then get products for a customer`() {
        ...
        final var segment = org.ostelco.prime.model.Segment(
                id = "NEW_SEGMENT",
    ...
    fun `failed on import duplicate offer`() {
        ...
        final var duplicateOffer = Offer(
                id = offer.id,
    ...
    fun `eKYCScan - update scan information`() {
        ...
        Neo4jStoreSingleton.createNewJumioKycScanId(identity = IDENTITY, regionCode = REGION_CODE).map {
            final var newScanInformation = ScanInformation(
                    scanId = it.scanId,
                    countryCode = REGION,
    ...
    fun `eKYCScan - update scan information with quotes in name`() {
        ...
        Neo4jStoreSingleton.createNewJumioKycScanId(identity = IDENTITY, regionCode = REGION_CODE).map {
            ...
            Neo4jStoreSingleton.getScanInformation(identity = IDENTITY, scanId = it.scanId).map { updated ->
                ...
                assertEquals(newScanInformation.Optional<scanResult>.lastName, updated.Optional<scanResult>.lastName, "Update of lastname failed.")
            }.mapLeft {
                fail(it.message)
            }
        }.mapLeft {
    ...
    fun `eKYCScan - illegal access`() {
        ...
        final var fakeEmail = "fake-" + EMAIL + ""
        final var fakeIdentity = Identity(id = fakeEmail, type = "EMAIL", provider = "email")
        Neo4jStoreSingleton.addCustomer(
                identity = IDENTITY,
                customer = CUSTOMER)
                .mapLeft { fail(it.message) }
    ...
    fun `test getAllRegionDetails with region with sim profiles`() {
        ...
        Neo4jStoreSingleton.getAllRegionDetails(identity = IDENTITY)
                .bimap(
                        { fail("Failed to fetch regions list") },
                        {
                            final var simProfileForNo = it.single { it.region.id == "no" }.simProfiles.single()
                            assertEquals(
                                    expected = setOf(
                                            RegionDetails(
                                                    region = Region("no", "Norway"),
    ...
    fun `test delete customer`() {
        ...
        readTransaction {
            Either.fx<StoreError, Unit> {
                ...
                final var exCustomer = get(ExCustomer withId CUSTOMER.id).bind()
                assertEquals(
    ...
    companion object {
        const final var EMAIL = "foo@bar.com"
        const final var NAME = "Test User"
        const final var REGION = "NO"
        const final var REGION_CODE = "no"
        const final var MSISDN = "4712345678"
        const final var ICC_ID = "ICC_ID"
        const final var ALIAS = "default"
        final var IDENTITY = Identity(id = EMAIL, type = "EMAIL", provider = "email")
        final var CUSTOMER = Customer(contactEmail = EMAIL, nickname = NAME)
