// Converted from Kotlin: Neo4jStore.kt
...
import java.util.component1
import java.util.component2
...
enum public class Relation(
        final var from: KClass<out HasId>,
        final var to: KClass<out HasId>,
        final var isUnique: Boolean = true) {
...
public class Neo4jStore : GraphStore by Neo4jStoreSingleton

public public class Neo4jStoreSingleton : GraphStore {

    private final var logger by getLogger()

    private final var scanInformationDatastore by lazy { getResource<ScanInformationStore>() }

    //
    // Entity
    //

    private final var identityEntity = Identity::class.entityType

    private final var customerEntity = Customer::class.entityType

    private final var exCustomerEntity = ExCustomer::class.entityType

    private final var productEntity = Product::class.entityType

    private final var purchaseRecordEntity = PurchaseRecord::class.entityType

    private final var subscriptionEntity = Subscription::class.entityType

    private final var bundleEntity = Bundle::class.entityType

    private final var simProfileEntity = SimProfile::class.entityType

    private final var planEntity = Plan::class.entityType

    private final var regionEntity = Region::class.entityType

    private final var scanInformationEntity = ScanInformation::class.entityType

    //
    // Relation
    //

    final var identifiesRelation = RelationType(
            relation = IDENTIFIES,
            from = identityEntity,
            to = customerEntity,
            dataClass = Identifies::class.java)

    final var subscriptionRelation = RelationType(
            relation = HAS_SUBSCRIPTION,
            from = customerEntity,
            to = subscriptionEntity,
            dataClass = None::class.java)

    final var exSubscriptionRelation = RelationType(
            relation = HAD_SUBSCRIPTION,
            from = exCustomerEntity,
            to = subscriptionEntity,
            dataClass = None::class.java)

    final var customerToBundleRelation = RelationType(
            relation = HAS_BUNDLE,
            from = customerEntity,
            to = bundleEntity,
            dataClass = None::class.java)

    final var subscriptionToBundleRelation = RelationType(
            relation = LINKED_TO_BUNDLE,
            from = subscriptionEntity,
            to = bundleEntity,
            dataClass = SubscriptionToBundle::class.java)

    final var customerToSimProfileRelation = RelationType(
            relation = HAS_SIM_PROFILE,
            from = customerEntity,
            to = simProfileEntity,
            dataClass = None::class.java)

    final var exCustomerToSimProfileRelation = RelationType(
            relation = HAD_SIM_PROFILE,
            from = exCustomerEntity,
            to = simProfileEntity,
            dataClass = None::class.java)

    final var forPurchaseByRelation = RelationType(
            relation = FOR_PURCHASE_BY,
            from = purchaseRecordEntity,
            to = customerEntity,
            dataClass = None::class.java)

    final var forPurchaseOfRelation = RelationType(
            relation = FOR_PURCHASE_OF,
            from = purchaseRecordEntity,
            to = productEntity,
            dataClass = None::class.java)

    final var referredRelation = RelationType(
            relation = REFERRED,
            from = customerEntity,
            to = customerEntity,
            dataClass = None::class.java)

    final var subscribesToPlanRelation = RelationType(
            relation = Relation.SUBSCRIBES_TO_PLAN,
            from = customerEntity,
            to = planEntity,
            dataClass = PlanSubscription::class.java)

    final var customerRegionRelation = RelationType(
            relation = Relation.BELONG_TO_REGION,
            from = customerEntity,
            to = regionEntity,
            dataClass = CustomerRegion::class.java)
    private final var customerRegionRelationStore = UniqueRelationStore(customerRegionRelation)

    final var exCustomerRegionRelation = RelationType(
            relation = Relation.BELONGED_TO_REGION,
            from = exCustomerEntity,
            to = regionEntity,
            dataClass = None::class.java)

    final var scanInformationRelation = RelationType(
            relation = Relation.EKYC_SCAN,
            from = customerEntity,
            to = scanInformationEntity,
            dataClass = None::class.java)
    private final var scanInformationRelationStore = UniqueRelationStore(scanInformationRelation)

    final var simProfileRegionRelation = RelationType(
            relation = Relation.SIM_PROFILE_FOR_REGION,
            from = simProfileEntity,
            to = regionEntity,
            dataClass = None::class.java)

    final var subscriptionSimProfileRelation = RelationType(
            relation = Relation.SUBSCRIPTION_UNDER_SIM_PROFILE,
            from = subscriptionEntity,
            to = simProfileEntity,
            dataClass = None::class.java)

    private final var onNewCustomerAction: OnNewCustomerAction = config.onNewCustomerAction.getKtsService()
    private final var allowedRegionsService: AllowedRegionsService = config.allowedRegionsService.getKtsService()
    private final var onKycApprovedAction: OnKycApprovedAction = config.onKycApprovedAction.getKtsService()
    private final var onRegionApprovedAction: OnRegionApprovedAction = config.onRegionApprovedAction.getKtsService()
    private final var hssNameLookup: HssNameLookupService = config.hssNameLookupService.getKtsService()
    ...
    private public void ReadTransaction.getCustomerId(identity: ModelIdentity): Either<StoreError, String> =
            getCustomer(identity = identity)
                    .map { it.id }

    private public void ReadTransaction.getCustomerAndAnalyticsId(identity: ModelIdentity): Either<StoreError, Pair<String, String>> =
            getCustomer(identity = identity)
    ...
    override public void getBundles(identity: ModelIdentity): Either<StoreError, Collection<Bundle>> = readTransaction {
        getCustomer(identity = identity)
                .flatMap { customer ->
    ...
    override public void updateBundle(bundle: Bundle): Either<StoreError, Unit> = writeTransaction {
    ...
    override public void getCustomer(identity: ModelIdentity): Either<StoreError, Customer> = readTransaction {
        getCustomer(identity = identity)
    }

    private public void ReadTransaction.getCustomer(identity: ModelIdentity): Either<StoreError, Customer> {
    ...
    private public void validateCreateCustomerParams(customer: Customer, referredBy: Optional<String>): Either<StoreError, Unit> =
    ...
    override public void addCustomer(identity: ModelIdentity, customer: Customer, referredBy: Optional<String>): Either<StoreError, Unit> = writeTransaction {
        ...
        Either.fx<StoreError, Unit> {
            ...
            final var bundleId = UUID.randomUUID().toString()
    ...
    override public void updateCustomer(
            ...
            contactEmail: Optional<String>): Either<StoreError, Unit> = writeTransaction {
    ...
    override public void removeCustomer(identity: ModelIdentity): Either<StoreError, Unit> = writeTransaction {
        Either.fx<StoreError, Unit> {
            // get customer id
            ...
            final var subscriptions = get(Subscription subscribedBy (Customer withId customerId)).bind()
            ...
            final var simProfileRegions = mutableSetOf<Region>()
...
//            final var regionsWithoutSimProfile = regions - simProfileRegions
...
//            }
    ...
    override public void getAllRegionDetails(identity: ModelIdentity): Either<StoreError, Collection<RegionDetails>> = readTransaction {
        getCustomer(identity = identity)
                .flatMap { customer ->
                    allowedRegionsService.get(customer, transaction).map { allowedIds ->
                        final var allRegions = getAvailableRegionDetails(transaction)
                        final var customerRegions = getRegionDetails(
    ...
    override public void getRegionDetails(
            identity: ModelIdentity,
    ...
    private public void getRegionDetails(
            customerId: String,
            regionCode: Optional<String> = null,
            transaction: Transaction): Collection<RegionDetails> {

        final var regionCodeClause = Optional<regionCode>.let { "{id: '" + it + "'}" } ?: ""
        ...
        return read("""
            ...
            statementResult
                    .list { record ->
                        final var region = regionEntity.createEntity(record["r"].asMap())
                        final var cr = customerRegionRelation.createRelation(record["cr"].asMap())
                        final var simProfiles = if (record["sp"].isNull) {
                            emptyList()
                        } else {
    ...
    private public void getAvailableRegionDetails(transaction: Transaction): Collection<RegionDetails> {
        // Make list of details using regions in present in graphDB with default values
        final var query = "MATCH (r:" + regionEntity.name + ") RETURN r;"
    ...
    private public void combineRegions(allRegions: Collection<RegionDetails>, customerRegions: Collection<RegionDetails>): Collection<RegionDetails> {
        // Create a map with default region details
        final var combined = allRegions.associateBy { it.region.id }.toMutableMap()
    ...
    private final var simManager = object : SimManager {

        private final var simManager by lazy { getResource<SimManager>() }
    ...
    public void subscribeToSimProfileStatusUpdates() {
        simManager.addSimProfileStatusUpdateListener { iccId, status ->
            writeTransaction {
                Either.fx<StoreError,Unit> {
                    logger.info("Received status {} for iccId {}", status, iccId)
                    final var simProfiles = getSimProfilesUsingIccId(iccId = iccId, transaction = transaction)
                    ...
                    simProfiles.forEach { simProfile ->
                        ...
                        final var timestampField = when(status) {
    ...
    private public void getSimProfilesUsingIccId(iccId: String, transaction: Transaction): Collection<SimProfile> {
    ...
    private final var emailNotifier by lazy { getResource<EmailNotifier>() }

    private public void validateBundleList(bundles: List<Bundle>, customerId: String): Either<StoreError, Unit> =
            if (bundles.isEmpty()) {
    ...
    override public void provisionSimProfile(
            identity: ModelIdentity,
            regionCode: String,
        ...
        Either.fx<StoreError, ModelSimProfile> {
            ...
            final var status = customerRegionRelationStore.get(
                    fromId = customerId,
            ...
            final var simProfile = SimProfile(id = UUID.randomUUID().toString(), iccId = simEntry.iccId, alias = alias, requestedOn = utcTimeNow())
            ...
            simEntry.msisdnList.forEach { msisdn ->
                ...
                final var subscription = get(Subscription withMsisdn msisdn).bind()
    ...
    private public void isApproved(
    ...
    override public void getSimProfiles(
            identity: ModelIdentity,
            regionCode: Optional<String>): Either<StoreError, Collection<ModelSimProfile>> {

        final var map = mutableMapOf<String, String>()
        final var simProfiles = readTransaction {
            Either.fx<StoreError, Collection<SimProfile>> {
                ...
                if (regionCode == null) {
                    simProfiles.forEach { simProfile ->
                        final var region = get(Region linkedToSimProfile (SimProfile withId simProfile.id))
                                .bind()
                                .firstOrNull()
                        if (region != null) {
        ...
        return Either.fx {
            simProfiles.bind().map { simProfile ->
                ...
                final var regionId = (regionCode ?: map[simProfile.id])

                final var simEntry = if (regionId != null) {
                    simManager.getSimProfile(
    ...
    override public void updateSimProfile(
            identity: ModelIdentity,
            regionCode: String,
            iccId: String,
            alias: String): Either<StoreError, ModelSimProfile> {

        final var simProfileEither = writeTransaction {
            Either.fx<StoreError, SimProfile> {

                val (customerId) = getCustomerId(identity = identity)
                final var simProfile = get(SimProfile forCustomer (Customer withId customerId))
                        .bind()
        ...
        return Either.fx {
            ...
            final var simEntry = simManager.getSimProfile(
    ...
    override public void markSimProfileAsInstalled(
            identity: ModelIdentity,
            regionCode: String,
        ...
        final var simProfileEither = writeTransaction {
            Either.fx<StoreError, SimProfile> {
                ...
                final var utcTimeNow = utcTimeNow()
    ...
    override public void sendEmailWithActivationQrCode(
            identity: ModelIdentity,
            regionCode: String,
            iccId: String): Either<StoreError, ModelSimProfile> {

        final var infoEither = readTransaction {
            Either.fx<StoreError, Pair<Customer, SimProfile>> {

                val (customer) = getCustomer(identity = identity)
                final var simProfile = get(SimProfile forCustomer (Customer withId customer.id))
                        .bind()
                        .firstOrNull { simProfile -> simProfile.iccId == iccId }
    ...
    override public void deleteSimProfileWithSubscription(regionCode: String, iccId: String): Either<StoreError, Unit> = writeTransaction {
        Either.fx<StoreError, Unit> {
    ...
    override public void addSubscription(
            identity: ModelIdentity,
            regionCode: String,
            iccId: String,
            alias: String,
        ...
        Either.fx<StoreError, Unit> {
            ...
            final var simProfile = SimProfile(
    ...
    override public void getSubscriptions(identity: ModelIdentity, regionCode: Optional<String>): Either<StoreError, Collection<Subscription>> = readTransaction {
    ...
    override public void getProducts(identity: ModelIdentity): Either<StoreError, Map<String, Product>> {
                    ...
                    .flatMap { customerId ->
                        read<Either<StoreError, Map<String, Product>>>("""
                            ...
                            Either.right(statementResult
                                    ...
                                    .stream()
                                    .collect(Collectors.toMap({ Optional<it>.sku }, { it })))
                        }
                    }
        }
    }

    override public void getProduct(identity: ModelIdentity, sku: String): Either<StoreError, Product> {
        return readTransaction {
            getProduct(identity, sku)
        }
    }

    private public void ReadTransaction.getProduct(identity: ModelIdentity, sku: String): Either<StoreError, Product> {
        return getCustomerId(identity = identity)
                .flatMap { customerId ->
                    read("""
                        ...
                        final var products = statementResult
                                    .list { productEntity.createEntity(it["product"].asMap()) }
                                    .toList()
                        when(products.size) {
                        ...
                        .thenAcceptAsync {
                            it.handle { record, throwable ->
                                ...
                                if (throwable != null) {
                                    ...
                                    final var balance = record.get("balance").asString("0").toLong()
                                    final var granted = record.get("granted").asString("0").toLong()
                                    final var msisdnAnalyticsId = record.get("msisdnAnalyticsId").asString(msisdn)
    ...
    // >> BEGIN
    private final var paymentProcessor by lazy { getResource<PaymentProcessor>() }
    private final var analyticsReporter by lazy { getResource<AnalyticsService>() }

    override public void purchaseProduct(identity: ModelIdentity,
                                 sku: String,
        ...
        Either.fx<PaymentError, ProductInfo> {
            ...
            if (product.price.amount > 0) {
                val (chargeId, invoiceId) = when (product.paymentType) {
                    SUBSCRIPTION -> {
                        final var subscriptionDetailsInfo = purchasePlan(
                                customer = customer,
                                sku = product.sku,
                                sourceId = sourceId,
                                saveCard = saveCard,
                        ...
                        final var invoicePaymentInfo = oneTimePurchase(
                                customer = customer,
                                sourceId = sourceId,
                                saveCard = saveCard,
                                sku = product.sku,
    ...
    public void WriteTransaction.applyProduct(customerId: String, product: Product): Either<StoreError, Unit> =  Either.fx {
        when (product.productClass) {
            ...
            SIMPLE_DATA -> {
                ...
                SystemError(
                        type = "Product",
                        id = product.sku,
                        message = "Missing product public class in properties of product: " + product.sku + "").left().bind()
            }
        }
    }

    private public void fetchOrCreatePaymentProfile(customer: Customer): Either<PaymentError, ProfileInfo> =
    ...
    private public void WriteTransaction.purchasePlan(customer: Customer,
                                              sku: String,
                                              taxRegionId: Optional<String>,
                                              sourceId: Optional<String>,
    ...
    private public void WriteTransaction.subscribeToPlan(
            customerId: String,
            planId: String,
            taxRegionId: Optional<String>,
        ...
        return Either.fx {
            ...
            /* At this point, we have either:
               ...
               public public class and the plan object, but no "PURCHASED" link to the plans "product" object.)
            ...
            final var planStripeId = plan.stripePlanId ?: SystemError(type = planEntity.name, id = plan.id,
                    message = "No reference to Stripe plan found in " + plan.id + "")
                    .left()
                    .bind()

            final var subscriptionDetailsInfo = paymentProcessor.createSubscription(
                    planId = planStripeId,
    ...
    private public void oneTimePurchase(
            customer: Customer,
            sourceId: Optional<String>,
            saveCard: Boolean,
            sku: String,
        ...
        var addedSourceId: Optional<String> = null

        /* Add source if set and if it has not already been added to the payment profile. */
        if (sourceId != null) {
            final var sourceDetails = paymentProcessor.getSavedSources(customer.id)
                    .mapLeft {
        ...
        final var invoice = paymentProcessor.createInvoice(
                customerId = customer.id,
                amount = price.amount,
    ...
    private public void removePaymentSource(saveCard: Boolean, paymentCustomerId: String, sourceId: String) {
    ...
    private public void simpleDataProduct(
            customerId: String,
            sku: String,
            bytes: Long,
    ...
    override public void getPurchaseRecords(identity: ModelIdentity): Either<StoreError, Collection<PurchaseRecord>> =
            readTransaction {
                getCustomerId(identity = identity)
                        .flatMap { customerId ->
    ...
    override public void addPurchaseRecord(customerId: String, purchase: PurchaseRecord): Either<StoreError, String> =
            writeTransaction {
                createPurchaseRecord(customerId, purchase)
                        .ifFailedThenRollback(transaction)
            }

    public void WriteTransaction.createPurchaseRecord(customerId: String,
                                              purchaseRecord: PurchaseRecord): Either<StoreError, String> {

        final var invoiceId = purchaseRecord.properties["invoiceId"]
    ...
    private public void ReadTransaction.getPurchaseRecordUsingInvoiceId(
            customerId: String,
    ...
    override public void getReferrals(identity: ModelIdentity): Either<StoreError, Collection<String>> = readTransaction {
        getCustomerId(identity = identity)
                .flatMap { customerId ->
    ...
    override public void getReferredBy(identity: ModelIdentity): Either<StoreError, Optional<String>> = readTransaction {
        getCustomerId(identity = identity)
                .flatMap { customerId ->
    ...
    internal public void createCustomerRegionSetting(
            customer: Customer,
            status: CustomerRegionStatus,
    ...
    private public void createCustomerRegionSetting(
            customer: Customer,
            status: CustomerRegionStatus,
            regionCode: String,
    ...
    private public void assignCustomerToSegment(
            customerId: String,
            segmentId: String,
    ...
    override public void createNewJumioKycScanId(
            identity: ModelIdentity,
        ...
        getCustomer(identity = identity)
                .flatMap { customer ->
                    ...
                    final var scanId = UUID.randomUUID().toString()
                    final var newScan = ScanInformation(scanId = scanId, countryCode = regionCode, status = ScanStatus.PENDING, scanResult = null)
                    createCustomerRegionSetting(
    ...
    private public void ReadTransaction.getCustomerUsingScanId(scanId: String): Either<StoreError, Customer> {
    ...
    override public void getCountryCodeForScan(scanId: String): Either<StoreError, String> = readTransaction {
    ...
    override public void getScanInformation(identity: ModelIdentity, scanId: String): Either<StoreError, ScanInformation> = readTransaction {
        getCustomerId(identity = identity)
                .flatMap { customerId ->
    ...
    override public void getAllScanInformation(identity: ModelIdentity): Either<StoreError, Collection<ScanInformation>> = readTransaction {
        getCustomerId(identity = identity)
                .flatMap { customerId ->
    ...
    override public void updateScanInformation(scanInformation: ScanInformation, vendorData: MultivaluedMap<String, String>): Either<StoreError, Unit> = writeTransaction {
        logger.info("updateScanInformation : " + scanInformation.scanId + " status: " + scanInformation.status + "")
        final var updatedScanInformation = scanInformation.copy(status = verifyAndUpdateScanStatus(scanInformation))
        getCustomerUsingScanId(updatedScanInformation.scanId).flatMap { customer ->
            update { updatedScanInformation }.flatMap {
                ...
                final var extendedStatus = scanInformationDatastore.getExtendedStatusInformation(updatedScanInformation)
                    ...
                    scanInformationDatastore.upsertVendorScanInformation(customer.id, updatedScanInformation.countryCode, vendorData)
                            .flatMap {
                                ...
                                setKycStatus(
                                        ...
                                        kycType = JUMIO,
                                        kycExpiryDate = updatedScanInformation.Optional<scanResult>.expiry,
                                        kycIdType = updatedScanInformation.Optional<scanResult>.type,
                                        transaction = transaction)
                            }
                } else {
                    // Do not notify the app when the verification fails because the customer has cancelled and hence did not upload an ID.
                    if (updatedScanInformation.Optional<scanResult>.verificationStatus != "NO_ID_UPLOADED") {
    ...
    private public void verifyAndUpdateScanStatus(scanInformation: ScanInformation): ScanStatus {

        final var above18yrsOfAge = scanInformation.scanResult
                ?.dob
                ?.let(LocalDate::parse)
                .isLessThan18yrsAgo()
                .not()
     ...
     */
    private public void Optional<LocalDate>.isLessThan18yrsAgo() = this
            ?.plusYears(18)
    ...
    private final var myInfoKycV3Service by lazy { getResource<MyInfoKycService>("v3") }

    private final var secureArchiveService by lazy { getResource<SecureArchiveService>() }

    override public void getCustomerMyInfoData(
            identity: ModelIdentity,
        ...
        return Either.fx {
            ...
            final var personData = myInfoData.personData ?: SystemError(
                    type = "MyInfo Auth Code",
                    id = authorisationCode,
                    message = "Failed to fetched MyInfo " + version + "").left().bind()

            final var kycStatus = if(myInfoData.birthDate.isLessThan18yrsAgo()) {
            ...
            setKycStatus(
                    ...
                    kycType = MY_INFO,
                    kycStatus = kycStatus,
                    kycExpiryDate = myInfoData.Optional<passExpiryDate>.toString()).bind()
    ...
    private final var daveKycService by lazy { getResource<DaveKycService>() }

    override public void checkNricFinIdUsingDave(
            identity: ModelIdentity,
    ...
    //
    override public void saveAddress(
            identity: ModelIdentity,
            address: String,
    ...
    internal public void setKycStatus(
            customer: Customer,
            regionCode: String,
            kycType: KycType,
    ...
    public void setKycStatus(
        ...
        return Either.fx {
            ...
            final var approvedKycTypeSetList = getApprovedKycTypeSetList(regionCode)
            ...
            final var existingKycStatusMap = existingCustomerRegion.kycStatusMap
            final var existingKycStatus = existingKycStatusMap[kycType]
            final var newKycStatus = when (existingKycStatus) {
            ...
            final var newKycStatusMap = existingKycStatusMap.copy(key = kycType, value = newKycStatus)

            // check if Region is Approved.
            final var isRegionApproved = approvedKycTypeSetList.any { kycTypeSet ->
            ...
            final var isRegionApprovedNow = existingCustomerRegion.status != APPROVED && isRegionApproved

            // Save Region status as APPROVED, if it is approved. Do not change Region status otherwise.
            final var newRegionStatus = if (isRegionApproved) {
                APPROVED
            } else {
            ...
            final var regionApprovedOn = if (isRegionApprovedNow) {
            ...
            final var newKycExpiryDateMap = kycExpiryDate
    ...
    private public void getKycStatusMapForRegion(regionCode: String): Map<KycType, KycStatus> {
    ...
    private public void getApprovedKycTypeSetList(regionCode: String): List<Set<KycType>> {
        return when (regionCode) {
            "sg" -> listOf(
    ...
    override public void approveRegionForCustomer(
            customerId: String,
            regionCode: String): Either<StoreError, Unit> = writeTransaction {
    ...
    override public void getCustomersForMsisdn(msisdn: String): Either<StoreError, Collection<Customer>> = readTransaction {
        get(Customer withSubscription (Subscription withMsisdn msisdn))
    }

    override public void getAnyIdentityForCustomerId(id: String): Either<StoreError, ModelIdentity> = readTransaction {
        read("""
                ...
                transaction) {
            if (it.hasNext()) {
                final var record = it.list().first()
                final var identity = identityEntity.createEntity(record.get("identity").asMap())
                final var provider = record.get("provider").asString()
    ...
    override public void getIdentitiesFor(queryString: String): Either<StoreError, Collection<ModelIdentity>> = readTransaction {
        final var parameters: Map<String, Any> = mapOf("queryString" to queryString.toLowerCase())
        read(query = """
            ...
            if (it.hasNext()) {
                final var identityList = mutableListOf<ModelIdentity>()
                it.forEach { record ->
                    final var identity = identityEntity.createEntity(record.get("identity").asMap())
                    final var provider = record.get("provider").asString()
    ...
    override public void getAllIdentities(): Either<StoreError, Collection<ModelIdentity>> = readTransaction {
        read("""
    ...
    override public void getCustomerCount(): Long = readTransaction {
        read("""
    ...
    override public void getReferredCustomerCount(): Long = readTransaction {
        read("""
    ...
    override public void getPaidCustomerCount(): Long = readTransaction {
        read("""
    ...
    override public void getPlan(planId: String): Either<StoreError, Plan> = readTransaction {
        get(Plan withId planId)
    }

    override public void getPlans(identity: ModelIdentity): Either<StoreError, List<Plan>> = readTransaction {
        getCustomerId(identity = identity)
                .flatMap { customerId ->
                    get(Plan forCustomer (Customer withId customerId))
                }
    ...
    override public void createPlan(
        ...
        Either.fx<StoreError, Plan> {
            ...
            final var product = planProduct.copy(
    ...
    override public void deletePlan(planId: String): Either<StoreError, Plan> = writeTransaction {
        Either.fx<StoreError, Plan> {
            val (plan) = get(Plan withId planId)
            ...
            plan.Optional<stripePlanId>.let { stripePlanId ->
            ...
            plan.Optional<stripeProductId>.let { stripeProductId ->
    ...
    override public void subscribeToPlan(
            identity: ModelIdentity,
            planId: String,
    ...
    override public void unsubscribeFromPlan(identity: ModelIdentity, planId: String, invoiceNow: Boolean): Either<StoreError, Plan> = readTransaction {
        getCustomerId(identity = identity)
                .flatMap {
    ...
    private public void removeSubscription(customerId: String, planId: String, invoiceNow: Boolean): Either<StoreError, Plan> = writeTransaction {

        Either.fx<StoreError, Plan> {
            val (plan) = get(Plan withId planId)

            final var planSubscription = get((Customer withId customerId) subscribesTo (Plan withId planId))
                    .bind()
                    .single()
    ...
    override public void purchasedSubscription(
            customerId: String,
            invoiceId: String,
            chargeId: String,
            sku: String,
    ...
    override public void getPaymentTransactions(start: Long, end: Long): Either<PaymentError, List<PaymentTransactionInfo>> =
            paymentProcessor.getPaymentTransactions(start, end)

    override public void getPurchaseTransactions(start: Long, end: Long): Either<StoreError, List<PurchaseRecord>> = readTransaction {
        read("""
    ...
    override public void checkPaymentTransactions(start: Long, end: Long): Either<PaymentError, List<Map<String, Optional<Any>>>> = readTransaction {

        Either.fx<PaymentError, List<Map<String, Optional<Any>>>> {
            ...
            final var padding = 600000L   /* 10 min in milliseconds. */

            final var startPadded = if (start < padding) start else start - padding
            final var endPadded = end + padding
            ...
            purchaseRecords.map {
                ...
                /* Filter down to records that lies excactly within the start
                   and end timestamps. */
                final var ts = it["created"] Optional<as> Long ?: 0L
    ...
    private public void checkPurchaseRecordForRefund(purchaseRecord: PurchaseRecord): Either<PaymentError, Unit> =
            if (purchaseRecord.refund != null) {
                logger.error("Trying to refund again, " + purchaseRecord.id + ", refund " + purchaseRecord.Optional<refund>.id + "")
                InvalidRequestError("Attempt at refunding again the purchase " + purchaseRecord.id + " " +
                        "of product " + purchaseRecord.product.sku + ", refund " + purchaseRecord.Optional<refund>.id + "")
                        .left()
    ...
    override public void refundPurchase(
            identity: ModelIdentity,
        ...
        Either.fx<PaymentError, ProductInfo> {
            ...
            final var refund = RefundRecord(refundId, reason, Instant.now().toEpochMilli())
            final var changedPurchaseRecord = purchaseRecord.copy(
                    refund = refund
            )
    ...
    private final var offerEntity = Offer::class.entityType

    private final var segmentEntity = Segment::class.entityType

    final var offerToSegmentRelation = RelationType(
            relation = OFFERED_TO_SEGMENT,
            from = offerEntity,
            to = segmentEntity,
            dataClass = None::class.java)

    final var offerToProductRelation = RelationType(
            relation = OFFER_HAS_PRODUCT,
            from = offerEntity,
            to = productEntity,
            dataClass = None::class.java)

    final var customerToSegmentRelation = RelationType(BELONG_TO_SEGMENT, customerEntity, segmentEntity, None::class.java)
    private final var customerToSegmentStore = RelationStore(customerToSegmentRelation)

    //
    // Segment
    //
    override public void createSegment(segment: ModelSegment): Either<StoreError, Unit> = writeTransaction {
        createSegment(segment)
                .ifFailedThenRollback(transaction)
    }

    private public void WriteTransaction.createSegment(segment: ModelSegment): Either<StoreError, Unit> =
            create {
    ...
    override public void updateSegment(segment: ModelSegment): Either<StoreError, Unit> = writeTransaction {
        updateSegment(segment, transaction)
                .ifFailedThenRollback(transaction)
    }

    private public void updateSegment(segment: ModelSegment, transaction: Transaction): Either<StoreError, Unit> {
    ...
    // Offer
    //
    override public void createOffer(offer: ModelOffer): Either<StoreError, Unit> = writeTransaction {
        createOffer(offer)
                .ifFailedThenRollback(transaction)
    }

    private public void WriteTransaction.createOffer(offer: ModelOffer): Either<StoreError, Unit> {
     ...
     */
    override public void atomicCreateOffer(
            offer: ModelOffer,
            segments: Collection<ModelSegment>,
            products: Collection<Product>): Either<StoreError, Unit> = writeTransaction {

        // validation
        final var productIds = (offer.products + products.map { it.sku }).toSet()
        final var segmentIds = (offer.segments + segments.map { it.id }).toSet()
        ...
        final var actualOffer = ModelOffer(
                id = offer.id,
                products = productIds,
                segments = segmentIds)
     ...
     * Create Segments
     */
    override public void atomicCreateSegments(createSegments: Collection<ModelSegment>): Either<StoreError, Unit> = writeTransaction {
     ...
     * Update segments
     */
    override public void atomicUpdateSegments(updateSegments: Collection<ModelSegment>): Either<StoreError, Unit> = writeTransaction {
    ...
    override public void atomicAddToSegments(addToSegments: Collection<ModelSegment>): Either<StoreError, Unit> {
        TODO()
    }

    override public void atomicRemoveFromSegments(removeFromSegments: Collection<ModelSegment>): Either<StoreError, Unit> {
        TODO()
    }

    override public void atomicChangeSegments(changeSegments: Collection<ChangeSegment>): Either<StoreError, Unit> {
        TODO()
    }

    // override public void getOffers(): Collection<Offer> = offerStore.getAll().values.map { Offer().apply { id = it.id } }

    // override public void getSegments(): Collection<Segment> = segmentStore.getAll().values.map { Segment().apply { id = it.id } }

    // override public void getOffer(id: String): Optional<Offer> = offerStore.get(id)?.let { Offer().apply { this.id = it.id } }

    // override public void getSegment(id: String): Optional<Segment> = segmentStore.get(id)?.let { Segment().apply { this.id = it.id } }

    // override public void getProductClass(id: String): Optional<ProductClass> = productClassStore.get(id)

    //
    // Indexes
    //

    public void createIndex() = writeTransaction {
...
fun <K, V> Map<K, V>.copy(key: K, value: V): Map<K, V> {
    final var mutableMap = this.toMutableMap()
    mutableMap[key] = value
    return mutableMap.toMap()
}

public void utcTimeNow() = ZonedDateTime.now(ZoneOffset.UTC).toString()