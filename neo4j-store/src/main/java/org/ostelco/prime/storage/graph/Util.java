...
final var adminStore by lazy { getResource<AdminDataSource>() }
...
public void createProduct(sku: String, taxRegionId: Optional<String> = null): Product {

    val (bytes, price) = sku.split("_")

    final var amount = (price.substring(0, price.length - 3).toFloat() * 100).toInt()
    final var currency = price.substring(price.length - 3)
 ...
 */
public void formatMoney(amount: Int, currency: String): String = DecimalFormat("#,###.##")
        .format(amount / 100.0) + " " + currency.toUpperCase() + ""

public void formatMoney(price: Price): String = formatMoney(price.amount, price.currency)
